pro readgroundmeasurement_surfrad_landsat2
  ;********************define the variable*******************************
  ;totapath='C:\Users\Angela.Erb\Dropbox\ValidationData\SURFRAD\' ;path for each site
  totapath='/lovells/data02/arthur.elmes/surfrad_2020/' ;path for each site
  sitepaname=['ft_peck','dsrt_rk','table_mnt']
  sitename=['\fpk','\drk','\bld'] ;name of the site on each data file
  tdiff=[7.00,6.00,5.00,5.00]
  title=['month', 'day' ,'year' ,'gnd_mean' ,'gnd_sdev' ,'dir/dif_ratio' ,'diffuse/(dir+diff)','zen' ]

  for siteid = 0, 2 do begin
    for yid =2020, 2020 do begin
      year=yid
      print,year
      ...
      for dayi = 0, daynum-1 do begin
        ...
        fullname=sitepath+yearpath+sitename(siteid)+yearname+dayname(dayi)+'.dat' ; the name for the file
        ...
        if ft eq 1 then begin
          ...
          id11_1=where(dt ge 10.01 and dt le 11.01)
      ...
      ;=[month, dayout ,yeararr ,gnd_mean ,gnd_sdev ,snow ,cfraction ,dir_dif_ratio , skyl]

      outname2=totapath+'results\'+sitepaname(siteid)+string(year,format='(i04)')+'result.txt'
