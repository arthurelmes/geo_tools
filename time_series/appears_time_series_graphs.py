...
def year_vs_avg_plot_anom(years, aoi_name, csv_path):
    ...
    ax_comb_mean_anom.plot(years.index[:-85], years['base_mean'][:-85], label='2000-2018 Mean +/- 1 SD',
                           color='slateblue', alpha=0.5)
    plt.fill_between(years.index[:-85], years['base_mean'][:-85] - years['base_sd'][:-85],
                     years['base_mean'][:-85] + years['base_sd'][:-85], color='lightgrey')
...
def main():
    ...
    szn_mask = (ts_df['Date'].dt.month > 3) & (ts_df['Date'].dt.month < 9)
    ...
    # All this stuff is for selecing a certain year to highlight. Keep for now, but fix in future, make func
    ...
    #
    # # This whole thing is hacky -- would be better to replace with native matplotlib plots, maybe
    ...
    # plt.savefig(fname=str(csv_path[:-4]  + 'white_sky_time_series_stacked.png'), dpi=300, bbox_inches='tight')
